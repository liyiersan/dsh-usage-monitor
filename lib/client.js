...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function fetchDashboard(force = false) {
			const url = force ? `${DATA_ENDPOINT}?force=1` : DATA_ENDPOINT;
...
`;
		...
		function UsagePanel(props) {
			...
			// 用量变化时上报（节流：内容未变则跳过），用于跨会话累计。
			React.useEffect(() => {
				if (!sessionId) return undefined;
				const totals = totalTokens(usage);
				const signature = `${usage.uncachedInputTokens ?? 0}|${usage.cacheReadTokens ?? 0}|${usage.cacheWriteTokens ?? 0}|${usage.outputTokens ?? 0}|${model}`;
				if (totals <= 0 || signature === reportedRef.current) return undefined;
				const timer = setTimeout(() => {
					reportedRef.current = signature;
					reportUsage(sessionId, model, {
						...
						.then((result) => {
							if (result?.totals && dashboard) setDashboard((prev) => (prev ? { ...prev, totals: result.totals } : prev));
						})
						.catch(() => {});
				}, 1200);
				return () => clearTimeout(timer);
			}, [sessionId, model, usage.uncachedInputTokens, usage.cacheReadTokens, usage.cacheWriteTokens, usage.outputTokens]);
			...
			return h(
				...
				h(
					...
					h(Card, {
						label: '本会话花费',
						value: formatCNY(session.costCNY),
						note: `${formatTokens(totalTokens(usage))} tokens${hitRate === null ? '' : ` · 缓存命中 ${(hitRate * 100).toFixed(0)}%`}`,
					}),
					h(Card, {
						label: '累计花费',
				...
				h(
					'div',
					{ className: 'umsm-foot' },
					`高峰时段为北京时间周一至周五 9:00-12:00、14:00-18:00，其余为空闲时段（价格为高峰的一半）。金额由 token 用量按官方单价本地估算，与平台账单口径一致但可能有极小的取整差异。`,
				),
			);
		}
		...
		function UsageDock(props) {
			...
			const hasSpend = session.costCNY > 0 || totalTokens(usage) > 0;
			...
			return h(
				...
				hasSpend ? h('span', { style: { flexShrink: 0 } }, `本会话 ${formatCNY(session.costCNY)}`) : null,
		...
		exports.__internal = { PRICING, computeCost, normalizeModel, isPeak, totalTokens, cacheHitRate };
