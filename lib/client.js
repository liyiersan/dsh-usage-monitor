...
`;
		...
		function UsagePanel(props) {
			...
			const refresh = React.useCallback(
				async (force) => {
					...
					try {
						...
						const data = await fetchDashboard(force, sessionId);
						setDashboard(data);
						setError(null);
					} catch (err) {
			...
			useUsageReporter(props, usage, model, (result) => {
				...
				if (result?.totals) setDashboard((prev) => (prev ? { ...prev, totals: result.totals } : prev));
			});
		...
		function UsageDock(props) {
			...
			React.useEffect(() => {
				let alive = true;
				// 带 sessionId：服务端会返回该会话在账本里的已计价金额（跨模型准确）。
				const load = () =>
					fetchDashboard(false, sessionId)
						.then((data) => {
		...
		exports.__internal = { PRICING, computeCost, normalizeModel, isPeak, totalTokens, cacheHitRate, sessionCostOf };
