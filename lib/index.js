...
export function apply(ctx) {
	...
	function summarize() {
		...
		for (const id of ids) {
			...
			byModel[model] = byModel[model] ?? { costCNY: 0, sessions: 0 };
			byModel[model].costCNY += Number(record?.costCNY ?? 0);
	...
	async function handleReport(payload) {
		...
		ledgerDirty = true;
		void flushLedger();
		return { ok: true, costCNY: book.sessions[sessionId].costCNY, totals: summarize() };
	}
